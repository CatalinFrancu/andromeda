...
int AlphaBetaAgent::alphaBeta(Board b, int depth, int alpha, int beta) {
  ...
  bool swapped = bringBestMoveFirst(rec, depth, &moveGen);

  Board new_b[TT_PREFETCH_MOVES];
  for (int i = 0; i < TT_PREFETCH_MOVES; i++) {
    if (moveGen.exists(i)) {
  ...
  short bestMove = 0;
  for (int i = 0; moveGen.exists(i); i++) {
    ...
    if (moveGen.exists(i + TT_PREFETCH_MOVES)) {
